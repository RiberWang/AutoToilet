...
+ (MBProgressHUD *)showText:(NSString *)text {
    MBProgressHUD *hud = [MBProgressHUD showText:text toView:nil];
    return hud;
}
...
+ (MBProgressHUD *)showText:(NSString *)text toView:(UIView *)view {
    ...
    hud.label.text = text;
    hud.label.font = [UIFont systemFontOfSize:W(16.0f)];
    hud.label.numberOfLines = 0;
    hud.minSize = CGSizeMake(W(80), H(40));
...
+ (MBProgressHUD *)show:(NSString *)text icon:(NSString *)icon view:(UIView *)view
{
    ...
    if ([Helper widthOfString:text font:hud.label.font height:0] > W(100)) {
        hud.mode = MBProgressHUDModeText;
    } else {
        // 设置图片
